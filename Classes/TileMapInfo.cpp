...
void TileMapInfo::initMapInfoWithFile(const char* tmxFile)
{
	...
	mTMXTileMap->setPosition(Point((winSize.width - mapSize.width * mTMXTileMap->getScale()) / 2,
...
bool TileMapInfo::collisionTest(Rect rect)
{
	...
	gid = mTMXLayers[0]->getTileGIDAt(Point((int)(rect.getMinX() / tileSize.width),
	...
	if (gidToTileType[gid] != tileNone && gidToTileType[gid] != tileGrass)
		return true;
	gid = mTMXLayers[0]->getTileGIDAt(Point((int)(rect.getMaxX() / tileSize.width),
		(int)(MaxY / tileSize.height)));
	if (gidToTileType[gid] != tileNone && gidToTileType[gid] != tileGrass)
		return true;
	gid = mTMXLayers[0]->getTileGIDAt(Point((int)(rect.getMaxX() / tileSize.width),
